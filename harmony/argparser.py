...
class ArgParser:
	def __init__(self):
		...
		self.parser.add_argument('-d', '--destination', dest='destination', help="destination directory to move processed mp3 files (will be created if doesn't exist)", required=True)
		self.parser.add_argument('-x', '--mask', dest='mask', help="mask for new files (available keys: artist, album, title, tracknumber, genre, date)", required=True)
